...
CREATE TABLE IF NOT EXISTS tag (
    ...
    create_index(tx, "idx_tag_name", "tag", "name")?;
...
CREATE TABLE IF NOT EXISTS file (
    ...
    create_index(tx, "idx_file_fingerprint", "file", "fingerprint")?;
...
CREATE TABLE IF NOT EXISTS file_tag (
    ...
    create_index(tx, "idx_file_tag_file_id", "file_tag", "file_id")?;
    create_index(tx, "idx_file_tag_tag_id", "file_tag", "tag_id")?;
    create_index(tx, "idx_file_tag_value_id", "file_tag", "value_id")?;
...
/// Create an index on a single table column
fn create_index(tx: &mut Transaction, idx_name: &str, table: &str, column: &str) -> Result<()> {
    tx.execute(&format!("CREATE INDEX IF NOT EXISTS {} ON {}({})", idx_name, table, column))?;
