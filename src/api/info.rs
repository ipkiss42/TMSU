...
fn compute_stats(tx: &mut Transaction) -> Result<StatsOutput> {
    Ok(StatsOutput {
        tag_count: storage::tag::tag_count(tx)?,
        value_count: storage::value::value_count(tx)?,
        file_count: storage::file::file_count(tx)?,
        file_tag_count: storage::filetag::file_tag_count(tx)?,
    })
}

fn compute_usage(tx: &mut Transaction) -> Result<UsageOutput> {
    let tag_usage = storage::tag::tag_usage(tx)?;
